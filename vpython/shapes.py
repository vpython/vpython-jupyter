...
def rframe(pos=[0,0], width=1, height=None, rotate=0, thickness=None, 
    ...
    outer = rectangle(pos=pos, width=width, height=height)
    inner = rectangle(pos=pos, width=width - thickness, height=height - thickness)
    if rotate != 0:
...
def rectangle(pos=[0,0], width=1, height=None, rotate=0, thickness=0, 
                roundness=0, invert=False, scale=1, xscale=1, yscale=1):
    if height is None:
        height = width
    if thickness == 0:
        ...
        cp = rframe(pos=pos, width=width, height=height, rotate=rotate, thickness=thickness, 
                roundness=roundness, invert=invert, scale=scale, xscale=xscale, yscale=yscale)
    return cp

def cross(pos=[0,0], width=1, rotate=0, thickness=0.2, 
                roundness=0, invert=False, scale=1, xscale=1, yscale=1):
...
def trframe(pos=[0,0], width=2, height=1, top=None, rotate=0, thickness=None, 
                roundness=0, invert=False, scale=1, xscale=1, yscale=1):
    ...
    outer = trapezoid(pos=pos, width=width, height=height, top=top)
    angle = atan((width - top) / 2 / height)
    db = thickness / cos(angle)
    inner = trapezoid(pos=pos, width=width - db - thickness * tan(angle), 
...
def trapezoid(pos=[0,0], width=2, height=1, top=None, rotate=0, thickness=0, 
                roundness=0, invert=False, scale=1, xscale=1, yscale=1):
    w2 = width / 2
    h2 = height / 2
    ...
    if thickness == 0:
        ...
        cp = trframe(pos=pos, width=width, height=height, top=top, rotate=rotate, thickness=thickness, 
                roundness=roundness, invert=invert, scale=scale, xscale=xscale, yscale=yscale)
    return cp

def circframe(pos=[0,0], radius=0.5, np=npdefault, rotate=0, iradius=None,
                angle1=0, angle2=2*pi, scale=1, xscale=1, yscale=1):
    thickness = 0
    if iradius is None: iradius = radius * .8
    outer = circle(pos=pos, radius=radius, np=np, rotate=rotate, iradius=iradius,
    ...
    if angle1 == 0 and angle2 == 2*pi:
        ...
        radius = iradius    
    inner = circle(pos=pos, radius=radius, np=np, rotate=rotate, thickness=thickness, corner=corner,
                   angle1=angle1, angle2=angle2, scale=scale, xscale=xscale, yscale=yscale)
    if rotate != 0:
        outer = rotatecp(outer, pos=pos, rotate=rotate)
        inner = rotatecp(inner, pos=pos, rotate=rotate)
...
def circle(pos=[0,0], radius=0.5, np=npdefault, rotate=0, thickness=0, corner=None, iradius=None,
                angle1=0, angle2=2*pi, scale=1, xscale=1, yscale=1):
    if corner is None: corner = pos # where the two straight edges meet
    cp = []
    if thickness > 0:
        iradius = radius - radius*thickness
        cp = circframe(pos=pos, radius=radius, np=np, rotate=rotate, iradius=iradius, 
                angle1=angle1, angle2=angle2, scale=scale, xscale=xscale, yscale=yscale)
    else:
...
def arc(pos=[0,0], radius=0.5, np=npdefault, rotate=0, thickness=None, 
...
def ellipse(pos=[0,0], width=1, height=None, np=npdefault, rotate=0, thickness=0,
                angle1=0, angle2=2*pi, scale=1, xscale=1, yscale=1):
    if height is None: height = .5 * width
    yscale *= height/width
    radius = width
    return circle(pos=pos, radius=radius, np=np, rotate=rotate, 
                angle1=angle1, angle2=angle2, scale=scale, xscale=xscale, yscale=yscale)

# line is not called by paths.line()
def line(pos=[0,0], width=1, height=None, np=2, rotate=0, thickness=None,
...
def nframe(pos=[0,0], length=1, rotate=0, thickness=None, np=3, 
                roundness=0, invert=False, scale=1, xscale=1, yscale=1):
    if thickness is None:
        thickness = length * .1
    else:
    ...
    outer = ngon(pos=pos, np=np, length=length)
    angle = pi * (.5 - 1 / np)
    length2 = length - 2 * thickness / tan(angle)
    inner = ngon(pos=pos, np=np, length=length2)
    if rotate != 0:
...
def ngon(pos=[0,0], length=1, rotate=0, thickness=0, np=None, 
                roundness=0, invert=False, scale=1, xscale=1, yscale=1):
    cp = []
    ...
    if thickness == 0:
        ...
        cp = nframe(pos=pos, length=length, rotate=rotate, thickness=thickness, np=np,
                roundness=roundness, invert=invert, scale=scale, xscale=xscale, yscale=yscale)
    return cp

def triangle(pos=[0,0], length=1, rotate=0, thickness=0, 
                roundness=0, invert=False, scale=1, xscale=1, yscale=1):
    return ngon(pos=pos, np=3, length=length, rotate=rotate-pi/6,
               roundness=roundness, invert=invert, scale=scale,
               xscale=xscale, yscale=yscale, thickness=thickness)

def pentagon(pos=[0,0], length=1, rotate=0, thickness=0, 
                roundness=0, invert=False, scale=1, xscale=1, yscale=1):
    return ngon(pos=pos, np=5, length=length, rotate=rotate-pi/10,
               roundness=roundness, invert=invert, scale=scale,
               xscale=xscale, yscale=yscale, thickness=thickness)

def hexagon(pos=[0,0], length=1, rotate=0, thickness=0, 
                roundness=0, invert=False, scale=1, xscale=1, yscale=1):
    return ngon(pos=pos, np=6, length=length, rotate=rotate,
               roundness=roundness, invert=invert, scale=scale,
               xscale=xscale, yscale=yscale, thickness=thickness)

def octagon(pos=[0,0], length=1, rotate=0, thickness=0, 
                roundness=0, invert=False, scale=1, xscale=1, yscale=1):
    return ngon(pos=pos, np=8, length=length, rotate=rotate+pi/8,
               roundness=roundness, invert=invert, scale=scale,
               xscale=xscale, yscale=yscale, thickness=thickness)

def sframe(pos=[0,0], rotate=0, thickness=None, radius=1, n=5, iradius=None,
                roundness=0, invert=False, scale=1, xscale=1, yscale=1):
    if iradius is None:
        iradius = .5 * radius
    if thickness is None:
    ...
    outer = star(pos=pos, n=n, radius=radius, iradius=iradius)
    inner = star(pos=pos, n=n, radius=radius - thickness, iradius=(radius - thickness) * iradius / radius)
    if rotate != 0:
        outer = rotatecp(outer, pos, rotate)
        inner = rotatecp(inner, pos, rotate)
...
def star(pos=[0,0], rotate=0, thickness=0, radius=1, n=5, iradius=None,
                roundness=0, invert=False, scale=1, xscale=1, yscale=1):
    ...
    if thickness == 0:
        ...
        cp = sframe(pos=pos, rotate=rotate, thickness=thickness, radius=radius, iradius=iradius,
                n=n, roundness=roundness, invert=invert, scale=scale, xscale=xscale, yscale=yscale)
    return cp

def points(pos=[], rotate=0, path=False, roundness=0, invert=False, scale=1, xscale=1, yscale=1):
    cp = pos
...
def gear(pos=(0,0), n=20, radius=1, phi=20, addendum=None, dedendum=None,
